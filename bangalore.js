...
module.exports = function solution(key_array) {
    ...
    for (let i = 2; i < values.length; i++) {
        const distance_from_left_hand = Math.abs(values[i] - left_hand);
        const distance_from_right_hand = Math.abs(values[i] - right_hand);
        ...
        if (distance_from_left_hand === distance_from_right_hand) {
            const left_hand_next = Math.abs(values[i + 1] - left_hand);
            const right_hand_next = Math.abs(values[i + 1] - right_hand);
